...
void main() {
  group('calculateChecksum (verified references)', () {
    test('EAN-13', () {
      expect(BarcodeValidator.calculateChecksum(BarcodeFormat.ean13, '400638133393'), 1);
    });
    test('EAN-8', () {
      expect(BarcodeValidator.calculateChecksum(BarcodeFormat.ean8, '9638507'), 4);
    });
    test('UPC-A', () {
    ...
    test('throws for formats without a numeric check digit', () {
      expect(() => BarcodeValidator.calculateChecksum(BarcodeFormat.code128, 'ABC'),
          throwsA(isA<BarcodeGenException>()));
      expect(() => BarcodeValidator.calculateChecksum(BarcodeFormat.qr, 'x'),
          throwsA(isA<BarcodeGenException>()));
    });
  });
  ...
  group('isValid*', () {
    test('EAN-13 accepts valid, rejects bad check / length / non-digit', () {
      ...
      expect(BarcodeValidator.isValidEAN13('4006381333930'), isFalse); // bad check
      expect(BarcodeValidator.isValidEAN13('400638133393'), isFalse);  // 12 digits
      expect(BarcodeValidator.isValidEAN13('40063813339A1'), isFalse); // non-digit
    });
    test('EAN-8 + UPC-A', () {
